...
const Room = () => {
  ...
  useEffect(() => {
    ...
    const newSocket = io('http://linksphere-backend-k60s.onrender.com/', {
      auth: { token }
    });
  ...
  const fetchUser = async () => {
    try {
      const res = await axios.get('http://linksphere-backend-k60s.onrender.com/api/auth/me');
      setUser(res.data);
    } catch (err) {
  ...
  const fetchRoomDetails = async () => {
    try {
      const res = await axios.get(
        `http://linksphere-backend-k60s.onrender.com/api/rooms/${roomId}/details`
      );
  ...
  const fetchMessages = async () => {
    try {
      const res = await axios.get(
        `http://linksphere-backend-k60s.onrender.com/api/rooms/${roomId}/messages`
      );
  ...
  const handleDeleteRoom = async () => {
    ...
    try {
      await axios.delete(
        `http://linksphere-backend-k60s.onrender.com/api/rooms/${roomId}`
      );
  ...
  const handleExtendExpiry = async (days) => {
    try {
      ...
      await axios.put(
        `http://linksphere-backend-k60s.onrender.com/api/rooms/${roomId}/extend-expiry`,
        { expiresIn: days },
        {
          headers: {
  ...
  const handleRemoveMember = async (userId) => {
    ...
    try {
      await axios.delete(
        `http://linksphere-backend-k60s.onrender.com/api/rooms/${roomId}/remove/${userId}`
      );
  ...
  const handleReaction = async (messageId, emoji) => {
    try {
      ...
      await axios.post(
        `http://linksphere-backend-k60s.onrender.com/api/rooms/message/${messageId}/react`,
        { emoji },
        {
          headers: {
            Authorization: `Bearer ${token}`
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(
        `http://linksphere-backend-k60s.onrender.com/api/rooms/${roomId}/upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
